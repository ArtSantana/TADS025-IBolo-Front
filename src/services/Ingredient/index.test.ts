...
const mockResultData: Array<IngredientModel> = []
...
const makeSut = (): IngredientService => {
...
describe('Cake Service', () => {
  test('Should call get with the correctly URL', async () => {
    ...
    expect(mockedAxios.get).toHaveBeenCalledWith(`${API_URL}/ingredients`)
  })
  ...
  test('Should call put with the correctly URL', async () => {
    ...
    expect(mockedAxios.put).toHaveBeenCalledWith(`${API_URL}/ingredients/${id}`)
  })
